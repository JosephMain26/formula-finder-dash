...
// Columns we can export (drop "actions" — not data)
...
export function ExportReportDialog({ jobs, companies }: ExportReportDialogProps) {
  ...
  // Keep company selection in sync if list changes
  useMemo(() => {
  ...
  function generatePdf() {
    ...
    doc.setFontSize(9);
    const rangeText = dateFrom || dateTo ? `Range: ${dateFrom || "…"} → ${dateTo || "…"}` : "Range: All dates";
    doc.text(rangeText, 14, 21);
    doc.text(`Total jobs: ${filtered.length}`, 14, 26);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <DialogFooter>
          ...
          <Button onClick={generatePdf} disabled={selectedColumns.size === 0}>
